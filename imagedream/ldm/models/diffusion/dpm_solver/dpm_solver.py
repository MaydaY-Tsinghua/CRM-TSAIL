                ...
                if isinstance(condition, dict):
                    ...
                    for k in condition:
                        if isinstance(condition[k], list):
                            c_in[k] = [
                                torch.cat([unconditional_condition[k][i], condition[k][i]])
                                for i in range(len(condition[k]))
                            ]
                        elif isinstance(condition[k], torch.Tensor):
                            c_in[k] = torch.cat([unconditional_condition[k], condition[k]])
                        else:
                            assert condition[k] == unconditional_condition[k]
                            c_in[k] = condition[k]
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        if method == 'adaptive':
            ...
            with torch.no_grad():
                ...
                vec_t = timesteps[1].expand((x.shape[0]))
                model_prev_list = [self.model_fn(x, vec_t)]
                t_prev_list = [vec_t]
                # Init the first `order` values by lower order multistep DPM-Solver.
                for init_order in range(1, order):
                    vec_t = timesteps[init_order+1].expand(x.shape[0])
                ...
                for step in range(order+1, steps+1):
